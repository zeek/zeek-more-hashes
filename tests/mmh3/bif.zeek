...
event zeek_init() &priority=0 {
	...
	assert r == 1898642774, fmt("expected 1898642774 got %s", r);
	print "\"zeek\"", r;
}
...
event zeek_init() &priority=-1 {
	...
	mmh3_hash_update(h, "zeek");
	local r = mmh3_hash_finish(h);
	assert r == 162293119, fmt("expected 162293119 got %s", r);
...
event zeek_init() &priority=-3 {
	...
	mmh3_hash_update(h, "e");
	local r = mmh3_hash_finish(h);
	assert r == 2907484762, fmt("expected 2907484762 got %s", r);
